...
char *C_HL_extensions[] = {".c", ".h", ".cpp", ".php", NULL};
...
int editor_rowcx_to_rx(erow *row, int cx)
{
    int rx = 0;
    int j;
    for (j = 0; j < cx; j++)
...
int editor_rowrx_to_cx(erow *row, int rx)
{
    int cur_rx = 0;
    int cx;
...
void editor_find_callback(char *query, int key)
{
    ...
    for (i = 0; i < E.numrows; i++)
    {
        ...
        if (match)
        {
            last_match = current;
            E.cy = current;
            E.cx = editor_rowrx_to_cx(row, match - row->render);
            E.rowoff = E.numrows;
...
void editor_scroll()
{
    ...
    if (E.cx < E.numrows)
    {
        E.rx = editor_rowcx_to_rx(&E.row[E.cy], E.cx);
    }
...
void editor_draw_status_bar(struct abuf *ab)
{
    ...
    int rlen = snprintf(rstatus, sizeof(rstatus), "%s | %d/%d", 
                E.syntax ? E.syntax->filetype: "no fit", E.cy + 1, E.numrows);
    if (len > E.screencols)
        len = E.screencols;
